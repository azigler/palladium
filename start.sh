...
echo "CREATE USER '$PALLADIUM_MARIADB_NEXTCLOUD_USER'@'nextcloud.$PALLADIUM_DOCKER_NETWORK' IDENTIFIED BY '$PALLADIUM_NEXTCLOUD_PASSWORD';
...
echo "CREATE USER '$PALLADIUM_MARIADB_WIKIJS_USER'@'wikijs.$PALLADIUM_DOCKER_NETWORK' IDENTIFIED BY '$PALLADIUM_WIKIJS_PASSWORD';
...
sed -i "s/POSTMASTER_ADDRESS=/POSTMASTER_ADDRESS=$PALLADIUM_ADMIN_EMAIL/" mailserver.env
